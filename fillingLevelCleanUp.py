...
def app(df, slc_places):
    ...
    """ for z in df['fillingLevel.percent']:
        if math.isnan(z):
            df.loc[z, 'fillingLevel.percent'] = 0 """

    ### Cleanup wenn place = startplace und fillingState sich am start_place ändert zwischen messungen
    for p in slc_places:
        for i in range(1, len(df)-2):
            if df.loc[i, 'place.name'] == p and (df.loc[i, 'fillingLevel.state'] == 'empty' or df.loc[i, 'fillingLevel.state'] == 'in use') and df.loc[i-1, 'fillingLevel.state'] == 'Full':
                j = i + 1
